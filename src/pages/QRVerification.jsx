...
export default function QRVerification() {
  ...
  // Build the ID-card PDF (card image on an A4 page) — shared by save & print.
  const buildIDCardDoc = async () => {
    const canvas = await renderIDCardCanvas()
  ...
  // Save the ID card as a PDF file.
  const downloadIDCardPDF = async () => {
    if (!selected) return
    const doc = await buildIDCardDoc()
    logMutation.mutate({ resident_id: selected.id, purpose: 'Barangay ID (PDF)' })
    doc.save(`${selected.resident_no}_BarangayID.pdf`)
  }

  // Print the ID card directly: opens the generated PDF straight to the print
  // dialog (no manual download, no crude browser-HTML print).
  const printIDCardPDF = async () => {
    if (!selected) return
    const doc = await buildIDCardDoc()
    logMutation.mutate({ resident_id: selected.id, purpose: 'Barangay ID (printed)' })
    doc.autoPrint()
...
<div class="footer">
  ...
  const generateCertificatePDF = async () => {
    const built = await buildCertificateDoc()
    if (!built) return
    logMutation.mutate({ resident_id: built.resident.id, purpose: `${built.title} (PDF)` })
    built.doc.save(`${built.resident.resident_no}_${built.title.replace(/\s+/g, '_')}.pdf`)
  }

  // Print the certificate directly: opens the PDF straight to the print dialog.
  const printCertificatePDF = async () => {
    const built = await buildCertificateDoc()
    if (!built) return
    logMutation.mutate({ resident_id: built.resident.id, purpose: `${built.title} (printed)` })
    built.doc.autoPrint()
    window.open(built.doc.output('bloburl'), '_blank')
  }
        ...
        <SectionCard title="Barangay ID with QR Code">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ flex: 1, opacity: selected ? 1 : 0.5, cursor: selected ? 'pointer' : 'not-allowed' }}
              disabled={!selected}
              onClick={printIDCardPDF}
            >
              🖨️ Print ID Card
            </button>
            <button
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '12px 18px', borderTop: '1px solid #E8E4DA', display: 'flex', gap: 8, justifyContent: 'flex-end', background: '#FAFAF7' }}>
              ...
              {certCtx ? (
                <>
                  <button className="btn btn-ghost" onClick={generateCertificatePDF}>⬇️ Download PDF</button>
                  <button className="btn btn-primary" onClick={printCertificatePDF}>🖨️ Print</button>
                </>
              ) : (
